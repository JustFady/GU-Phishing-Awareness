...
After form submission, the result page shows a browser-only capture report with the submitted email address, masked password values with character counts, timestamp, public IP when available, and browser-visible metadata. Password values are not stored or displayed in raw form.
